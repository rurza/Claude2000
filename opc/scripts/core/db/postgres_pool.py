...
def _is_embedded_mode() -> bool:
    """Check if using embedded PostgreSQL mode.

    Embedded mode is indicated by PGSERVER_PGDATA env var.
    """
    return bool(os.environ.get("PGSERVER_PGDATA"))


def _ensure_embedded_postgres_running() -> None:
    """Ensure embedded PostgreSQL server is running.

    Uses pgserver-venv with pg_ctl to start a persistent server.
    The unix_socket_directories is set to pgdata so connections work
    with the connection string format: postgresql://...?host=<pgdata>
    """
    pgdata = os.environ.get("PGSERVER_PGDATA")
    if not pgdata:
        return
    ...
    pgserver_venv = os.environ.get("PGSERVER_VENV", str(Path.home() / ".claude" / "pgserver-venv"))
    python_exe = Path(pgserver_venv) / "bin" / "python"

    if not python_exe.exists():
        _logger.warning(f"pgserver-venv not found at {pgserver_venv}")
        return

    # Use pg_ctl to start a persistent daemon with socket in pgdata directory
    # The -o flag passes options to postgres, including unix_socket_directories
    start_script = f"""
import pgserver
from pathlib import Path
pgdata = Path('{pgdata}')
pgserver.pg_ctl(
    ['start', '-l', str(pgdata / 'log'), '-o', f'-c unix_socket_directories={{pgdata}}'],
    pgdata=pgdata
)
"""
    try:
        ...
        _logger.warning(f"Error starting embedded PostgreSQL: {e}")


def get_connection_string() -> str:
    """Get PostgreSQL connection string from environment.

    Uses CLAUDE2000_DB_URL exclusively (self-contained, no generic DATABASE_URL).
    This is set by the wizard in ~/.claude/.env.

    For embedded mode, auto-starts the server if not running.

    Raises:
        ValueError: If CLAUDE2000_DB_URL is not set.
    """
    # Auto-start embedded postgres if configured
    if _is_embedded_mode():
        _ensure_embedded_postgres_running()

    # Self-contained: use CLAUDE2000_DB_URL only
    url = os.environ.get("CLAUDE2000_DB_URL")

    if url:
        return url

    raise ValueError(
        "CLAUDE2000_DB_URL not set. Run the Claude2000 installer:\n"
        "  cd ~/.claude && uv run python -m scripts.setup.wizard"
    )
