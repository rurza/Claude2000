...
### SymPy (Symbolic Math)
```bash
uv run python "~/.claude/scripts/cc_math/sympy_compute.py" <command> <args>
```
...
```bash
uv run python "~/.claude/scripts/cc_math/z3_solve.py" <command> <args>
```
...
### Pint (Units)
```bash
uv run python "~/.claude/scripts/cc_math/pint_compute.py" convert <value> <from_unit> <to_unit>
```
...
```bash
uv run python "~/.claude/scripts/cc_math/math_router.py" route "<natural language request>"
```
...
```
User: Solve x² - 5x + 6 = 0
Claude: uv run python "~/.claude/scripts/cc_math/sympy_compute.py" solve "x**2 - 5*x + 6" --var x
Result: x = 2 or x = 3
```
...
Claude: uv run python "~/.claude/scripts/cc_math/sympy_compute.py" eigenvalues "[[2,1],[1,2]]"
...
Claude: uv run python "~/.claude/scripts/cc_math/z3_solve.py" prove "x**2 + y**2 >= 2*x*y"
...
Claude: uv run python "~/.claude/scripts/cc_math/pint_compute.py" convert 26.2 miles kilometers
Result: 42.16 km
```
